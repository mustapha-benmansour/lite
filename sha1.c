...
static int init(lua_State * L){
    if (lua_type(L, 1)==LUA_TSTRING){
        ...
        if (!ctx) return luaL_error(L,strerror(ENOMEM));
...
static int reset(lua_State * L) {
    SHA1_CTX* hd = lua_touserdata(L, 1);
    SHA1Init(hd);
    return 0;
}

static int update(lua_State * L){
    SHA1_CTX* ctx = lua_touserdata(L, 1);
    size_t len;
    const uint8_t *data=(const uint8_t *)lua_tolstring(L,2,&len);
    SHA1Update(ctx,data,len);
    return 0;
}
...
extern int luaopen_lite_sha1(lua_State * L){
    ...
    lua_pushcfunction(L,update);lua_setfield(L, -2, "update");
    lua_pushcfunction(L,final);lua_setfield(L, -2, "final");
    lua_pushcclosure(L, init,1);
    return 1;
}